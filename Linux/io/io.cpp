 ...
 *   返回值:off_t 文件指针从头部开始计算总的偏移量
 *   off_t lseek(int fd, off_t offset, int whence);
 *   6.1 移动文件指针
 *        lseek(fd,0,SEEK_END);得到文件大小,并移动文件指针到尾部  lseek(fd,0,SEEK_CUR);得到当前文件指针位置 lseek(fd,0,SEEK_SET)文件指针移动到文件头部
 *   6.2 扩展文件
 ...
 *   ftruncate(fd,1024); 用 这个函数可以代替上面 lseek + write
 * 8.文件描述符复制和重定向
 *   int dup(int oldfd); 新旧文件描述符指向同一个文件，关闭其中一个另一个还可以继续使用
 *   int dup2(int fd1, int fd2);重定向，情况1，fd1和fd2指向不同的文件，调用dup2后fd2断开指向的文件并指向fd1指向的文件
 *   情况2，fd1指向文件，fd2 没有指向任何文件。此时相当与复制文件描述符。
 *   情况3，fd1和fd2指向相同文件，此时调用dup2相当与啥都没做
 *   
 *
