 ...
 *    3.1 构造函数种类
 *        有参构造和无参构造、普通构造和拷贝构造，这几种构造函数形状看下面
 *    3.2 构造函数调用方式
 ...
 *        Box create(); 函数返回的对象是匿名对象，用同一类型的变量接收匿名对象不会调用拷贝构造函数 例如 Box b = create(); 不会调用拷贝构造函数
 *    3.4 拷贝构造函数的调用时机
 ...
 * 4. 深拷贝和浅拷贝
 *    编译器提供的拷贝构造函数叫做浅拷贝，拷贝原理，把对象1的属性值复制给对象2的属性，但是该例中height是堆区，当对象1执行析构函数的时候
